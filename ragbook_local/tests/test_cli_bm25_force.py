...
def test_bm25_rebuild_force_overwrites(tmp_path, monkeypatch):
    ...
    # Without --force the command should fail
    res = runner.invoke(app, ["bm25-rebuild", "--config", str(cfg_file), "--output", str(out_path)])
    assert res.exit_code != 0, res.output

    # With --force it should succeed and overwrite file with a valid pickle
    res2 = runner.invoke(app, ["bm25-rebuild", "--config", str(cfg_file), "--output", str(out_path), "--force"]) 
